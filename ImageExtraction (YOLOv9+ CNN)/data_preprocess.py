    ...
    for bbs in img.findall ('taggedRectangles'):
      for bb in bbs:
        ...
        if ' ' in bb[0].text.lower():
          continue
        bbs_of_img.append([
            float(bb.attrib['x']) ,float(bb.attrib['y']), float(bb.attrib['width']), float(bb.attrib['height'] )])
        labels_of_img.append(bb[0].text.lower())
    img_path = os.path.join(root_dir, img[0].text.lower())
    img_paths.append(img_path)
    img_sizes.append([int(img[1].attrib['width']), int(img[1].attrib['height'])])
...
def save_data(data, src_img_dir, save_dir): # Create folder if not exists
